...
interacts with the KSWAP Protocol using one specific set of tools (`truffle` + `npm` + `mocha`).
...
In order to reference the KSWAP contracts, you should use the npm artifacts we deploy containing the core and
...
[`@kimchiswap/v1-core`](https://www.npmjs.com/package/@kimchiswap/v1-core) and 
[`@kimchiswap/v1-periphery`](https://www.npmjs.com/package/@kimchiswap/v1-periphery) packages.

```shell script
npm i --save @kimchiswap/v1-core
npm i --save @kimchiswap/v1-periphery
```

If you check the `node_modules/@kimchiswap` directory, you can now find the KSWAP contracts. 

```shell script
moody@MacBook-Pro ~/I/u/demo> ls node_modules/@kimchiswap/v1-core/contracts
KswapV1ERC20.sol    KswapV1Pair.sol     libraries/
KswapV1Factory.sol  interfaces/           test/
moody@MacBook-Pro ~/I/u/demo> ls node_modules/@kimchiswap/v1-periphery/contracts/
KswapV1Migrator.sol  examples/              test/
KswapV1Router01.sol  interfaces/
KswapV1Router02.sol  libraries/
```
...
Now let's start with the constructor. You need to know where the `KswapV1Factory` is deployed in order to compute the
...
The [`KswapV1Library`](/docs/v1/smart-contracts/library/) has some helpful methods for this.

```solidity
pragma solidity ^0.6.6;

import './interfaces/ILiquidityValueCalculator.sol';
import '@kimchiswap/v1-periphery/contracts/libraries/KswapV1Library.sol';
import '@kimchiswap/v1-core/contracts/interfaces/IKswapV1Pair.sol';

contract LiquidityValueCalculator is ILiquidityValueCalculator {
    function pairInfo(address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB, uint totalSupply) {
        IKswapV1Pair pair = IKswapV1Pair(KswapV1Library.pairFor(factory, tokenA, tokenB));
...
Note you should only deploy the precompiled KSWAP contracts in the `build` directories for unit tests. 
...
This is problematic because in KSWAP we use the hash of the bytecode in the v1-periphery
[`KswapV1Library`](https://github.com/Kimchiswap/Kswap-V1-Periphery/blob/master/contracts/libraries/KswapV1Library.sol#L24),
to compute the pair address.

To get the bytecode for deploying KswapV1Factory, you can import the file via:

```javascript
const KswapV1FactoryBytecode = require('@kimchiswap/v1-core/build/KswapV1Factory.json').bytecode
```
