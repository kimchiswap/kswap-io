...
const APOLLO_QUERY = gql`
  {
    kswapFactory(id: "0x90AdEeeC27e184280763AA91CE25D13f7552Ac54") {
      totalVolumeUSD
      totalLiquidityUSD
      pairCount
    }
...
export const KSWAP_GLOBALS_24HOURS_AGO_QUERY = block => {
  let queryString = `
  query kswapFactory {
    kswapFactory(id: "0x90AdEeeC27e184280763AA91CE25D13f7552Ac54", block: { number: ${block} }) {
      totalVolumeUSD
      totalLiquidityUSD
      pairCount
...
const About = props => {
  ...
  useEffect(() => {
    async function getData() {
      let result = await client.query({
        query: KSWAP_GLOBALS_24HOURS_AGO_QUERY(oneDayBackBlock),

        fetchPolicy: 'cache-first'
      })
      if (result) {
        setOnedayResult(result?.data?.kswapFactory)
      }
    }
    if (oneDayBackBlock) {
      getData()
  ...
  if (data && oneDayResult) {
    const volume24Hour = parseFloat(data?.kswapFactory?.totalVolumeUSD) - parseFloat(oneDayResult?.totalVolumeUSD)
    ...
    KswapStats.liquidity = [
      ...
      }).format(data.kswapFactory.totalLiquidityUSD)
    ]
    KswapStats.exchanges = [Number.parseFloat(data?.kswapFactory?.pairCount)]
      ...
      <StyledAbout>
        <span style={{ marginTop: '5rem' }}>
          ...
          <StyledSectionFlex id="about" style={{ flexDirection: 'column' }}>
            <p>
              KSWAP empowers developers, liquidity providers and traders to participate in a financial marketplace
          ...
          <StyledSectionFlex id="contact" style={{ flexDirection: 'column' }}>
            ...
            <p>
              To get in touch, please email <a href="mailto:admin@kimchiswap.io">admin@kimchiswap.io</a>
            </p>

            <p>
              We encourage anyone facing issues with their wallet, transaction or KSWAP related question to join our
          ...
          <StyledSectionFlex id="jobs" style={{ flexDirection: 'column' }}>
            ...
            <p>
              KSWAP welcomes all qualified persons to apply regardless of race, religion, gender, gender identity or
